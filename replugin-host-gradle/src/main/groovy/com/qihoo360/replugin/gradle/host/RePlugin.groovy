    ...
    public void apply(Project project) {
        ...
        if (project.plugins.hasPlugin(AppPlugin)) {
            ...
            android.applicationVariants.all { variant ->
                ...
                def javaCompileTask = resolveJavaCompileTask(variant)
                if (javaCompileTask) {
                    javaCompileTask.dependsOn generateHostConfigTask
                }
                ...
                def processManifestTask = VariantCompat.getProcessManifestTask(variant)
                if (processManifestTask != null) {
                    hookProcessManifest(processManifestTask, newManifest)
                } else {
                    variant.outputs.each { output ->
                        def t = VariantCompat.getProcessManifestTask(output)
                        if (t != null) {
                            hookProcessManifest(t, newManifest)
                        }
                    }
                }
            }
...
//        println "${TAG} appID=${appID}"
    ...
    def hookProcessManifest(def processManifestTask, def newManifest) {
    ...
    def resolveJavaCompileTask(def variant) {
        try {
            if (variant.hasProperty('javaCompileProvider') && variant.javaCompileProvider != null) {
        ...
        try {
            ...
            if (provider != null) {
                return Task.class.isInstance(provider) ? provider : provider.get()
            }
        } catch (Throwable ignored) {
        }
        try {
