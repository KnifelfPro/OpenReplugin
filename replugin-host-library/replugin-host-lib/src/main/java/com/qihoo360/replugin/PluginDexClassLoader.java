...
public class PluginDexClassLoader extends DexClassLoader {
    ...
    private static final String MULTIDEX_LOCK = "multidex.lock";
    private static final String MULTIDEX_STAMP = "multidex.stamp";
    ...
    private void installMultiDexesBeforeLollipop(PluginInfo pi, String dexPath, ClassLoader parent) {
        ...
        File odexDir = pi.getExtraOdexDir();
        RandomAccessFile lockRaf = null;
        FileLock lock = null;
        try {
            FileUtils.forceMkdir(odexDir);
            lockRaf = new RandomAccessFile(new File(odexDir, MULTIDEX_LOCK), "rw");
            lock = lockRaf.getChannel().lock();

            boolean reuseExtracted = isMultiDexStampValid(dexPath, odexDir);
            List<File> dexFiles = getExtraDexFiles(pi, dexPath, reuseExtracted);
            ...
            allElements.add((Object[]) ReflectUtils.readField(pathList.getClass(), pathList, "dexElements"));

            String optimizedDirectory = odexDir.getAbsolutePath();
            for (File file : dexFiles) {
                if (LogDebug.LOG && RePlugin.getConfig().isPrintDetailLog()) {
                    LogDebug.d(TAG, "dex file:" + file.getName());
                }
                DexClassLoader extraLoader = new DexClassLoader(
                        file.getAbsolutePath(), optimizedDirectory, optimizedDirectory, parent);
                Object extraPathList = ReflectUtils.readField(clz, extraLoader, "pathList");
                allElements.add((Object[]) ReflectUtils.readField(extraPathList.getClass(), extraPathList, "dexElements"));
            }

            ReflectUtils.writeField(pathList.getClass(), pathList, "dexElements", combineArray(allElements));
            writeMultiDexStamp(dexPath, odexDir);

            if (LogDebug.LOG && RePlugin.getConfig().isPrintDetailLog()) {
                Object merged = ReflectUtils.readField(pathList.getClass(), pathList, "dexElements");
                LogDebug.d(TAG, "dexElements length:" + Array.getLength(merged));
            }
        } catch (Exception e) {
            if (LogDebug.LOG) {
                LogDebug.e(TAG, "installMultiDexesBeforeLollipop failed", e);
            }
        } finally {
            if (lock != null) {
                try {
            ...
            CloseableUtils.closeQuietly(lockRaf);
        }
    }

    private static String apkIdentity(String dexPath) {
        File apk = new File(dexPath);
        return apk.length() + ":" + apk.lastModified();
    }

    private static boolean isMultiDexStampValid(String dexPath, File odexDir) {
        File stamp = new File(odexDir, MULTIDEX_STAMP);
        if (!stamp.isFile()) {
            return false;
        }
        try {
            return apkIdentity(dexPath).equals(FileUtils.readFileToString(stamp, Charsets.UTF_8).trim());
        } catch (IOException e) {
            return false;
        }
    }

    private static void writeMultiDexStamp(String dexPath, File odexDir) throws IOException {
        FileUtils.writeStringToFile(new File(odexDir, MULTIDEX_STAMP), apkIdentity(dexPath), Charsets.UTF_8);
    }
     ...
     */
    private List<File> getExtraDexFiles(PluginInfo pi, String dexPath, boolean reuseExtracted) {
        ZipFile zipFile = null;
        try {
            if (pi != null) {
                zipFile = new ZipFile(dexPath);
                return traverseExtraDex(pi, zipFile, reuseExtracted);
            }
        } catch (Exception e) {
            if (LogDebug.LOG) {
                LogDebug.e(TAG, "getExtraDexFiles failed", e);
            }
        } finally {
     ...
     * @return the File list of the extra dexes
     */
    private static List<File> traverseExtraDex(PluginInfo pi, ZipFile zipFile, boolean reuseExtracted) {
        String dir = null;
        ...
        while (entries.hasMoreElements()) {
            ...
            if (name.contains("../") || !name.contains(".dex") || name.equals("classes.dex")) {
                continue;
            }
            try {
                if (dir == null) {
                    File extraDexDir = pi.getExtraDexDir();
                    dir = extraDexDir.getAbsolutePath();
                    dirCanonical = extraDexDir.getCanonicalPath();
                }
                String baseName = new File(name).getName();
                if (baseName.isEmpty() || baseName.charAt(0) == '/' || baseName.charAt(0) == '\\'
                        || new File(baseName).isAbsolute()) {
                    continue;
                }
                File file = new File(dir, baseName);
                if (!file.getCanonicalPath().startsWith(dirCanonical + File.separator)) {
                    continue;
                }
                if (!(reuseExtracted && sameExtractedDex(file, entry))) {
                    extractFile(zipFile, entry, file);
                }
                files.add(file);
            } catch (Exception e) {
                if (LogDebug.LOG) {
                    LogDebug.e(TAG, "traverseExtraDex failed: " + name, e);
                }
            }
        }
        return files;
    ...
    private static boolean sameExtractedDex(File file, ZipEntry entry) {
        long expected = entry.getSize();
        return file.isFile() && file.length() > 0 && (expected < 0 || file.length() == expected);
    }
