...
public class RePluginApplication extends Application {
    ...
    /**
     * Android 4.4 still needs the MultiDex library when the host exceeds 64K methods.
     * ART native multi-dex starts at API 21.
     */
    private void installMultiDexIfNeeded() {
        if (Build.VERSION.SDK_INT >= 21) {
            return;
        }
        try {
            Class<?> clz = Class.forName("androidx.multidex.MultiDex");
            clz.getMethod("install", Context.class).invoke(null, this);
        } catch (Throwable ignored) {
            try {
                Class<?> clz = Class.forName("android.support.multidex.MultiDex");
                clz.getMethod("install", Context.class).invoke(null, this);
