...
public class RePlugin {
    ...
    public static PluginInfo install(String path) {
        ...
        if (file.getName().startsWith("p-n-")) {
            try {
                String installCanonical = RePlugin.getConfig().getPnInstallDir().getCanonicalPath();
                String fileCanonical = file.getCanonicalPath();
                if (!fileCanonical.startsWith(installCanonical + File.separator)) {
                    if (LogDebug.LOG) {
                        LogDebug.e(TAG, "install: Must be installed from the specified path. Path=" + path + "; Allowed=" + installCanonical);
                    }
                    return null;
                }
            } catch (IOException e) {
    ...
    /**
     * 将指定插件的默认进程留在宿主 UI 进程。框架版本 ≥ 4 时，未加入白名单的插件默认进程会被映射到坑位进程。
     * 请在插件加载前调用（例如 {@link App#attachBaseContext}）。
     *
     * @param pluginOrPackage 插件包名或插件名
     * @since 3.1.1
     */
    public static void addMainPkg(String pluginOrPackage) {
        PluginProcessHost.addMainPkg(pluginOrPackage);
    }
    ...
    public static class App {
        ...
        public static void attachBaseContext(Application app, RePluginConfig config) {
            ...
            com.qihoo360.replugin.compat.HiddenApiCompat.exemptAll();
