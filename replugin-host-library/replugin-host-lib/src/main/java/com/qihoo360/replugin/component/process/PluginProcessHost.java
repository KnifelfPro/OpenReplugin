...
public class PluginProcessHost {
    ...
    /**
     * 框架版本 ≥ 4 时，插件默认进程（包名）会被映射到宿主坑位进程。
     * 加入白名单后，该插件默认进程仍留在宿主 UI 进程。
     * 可传插件包名或插件名。
     */
    private static final Set<String> sKeepInUiProcess = new CopyOnWriteArraySet<>();

    public static void addMainPkg(String pluginOrPackage) {
        if (TextUtils.isEmpty(pluginOrPackage)) {
            return;
        }
        sKeepInUiProcess.add(pluginOrPackage);
    }

    public static boolean isKeepInUiProcess(String pluginOrPackage) {
        return !TextUtils.isEmpty(pluginOrPackage) && sKeepInUiProcess.contains(pluginOrPackage);
    }
