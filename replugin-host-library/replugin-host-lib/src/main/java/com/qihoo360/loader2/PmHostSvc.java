        ...
        for (HashMap.Entry<String, List<IntentFilter>> entry : receiverFilterMap.entrySet()) {
            ...
            if (filters != null) {
                for (IntentFilter filter : filters) {
                    ...
                    // Honor plugin android:exported="false"; omit/true stays exported for system broadcasts
                    boolean exported = !filter.hasCategory(ManifestParser.RECEIVER_NOT_EXPORTED_CATEGORY);
                    IntentFilter toRegister = exported ? filter : ManifestParser.copyWithoutNotExportedMarker(filter);
                    com.qihoo360.replugin.compat.ReceiverCompat.registerReceiver(mContext, mReceiverProxy, toRegister, exported);
                }
            }
        }
    }
