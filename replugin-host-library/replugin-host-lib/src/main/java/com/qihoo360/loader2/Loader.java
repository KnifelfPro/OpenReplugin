...
//                }
            ...
            if (mClassLoader == null) {
                ...
                boolean dexMadeReadOnly = false;
                try {
                    if (dexFile.exists()) { // Android 14: DEX must not be writable
                        dexFile.setWritable(false);
                        dexFile.setReadOnly();
                        dexMadeReadOnly = true;
                    }
                    ...
                    if (dexMadeReadOnly && dexFile.exists()) {
...
//            }
    ...
    private List<String> getPluginProcessList() {
        ...
        processSet.add(pluginUIProcess);
        ...
        // 默认把插件包名进程映射到宿主坑位；白名单插件仍留在 UI 进程
        if (PluginProcessHost.isKeepInUiProcess(pluginUIProcess)
                || PluginProcessHost.isKeepInUiProcess(mPluginName)) {
